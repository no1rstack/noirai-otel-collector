...
func TestFieldUnmarshalJSON(t *testing.T) {
	cases := []struct {
		...
		},
		{
			...
			expected: Field{entry.NewResourceField("test1")},
		},
		{
			name:     "ResourceComplexField",
			input:    []byte(`"resource.test1.test2"`),
			expected: Field{entry.NewResourceField("test1", "test2")},
		},
		{
			name:     "ResourceBracketedField",
			input:    []byte(`"resource.test1['file.name']"`),
			expected: Field{entry.NewResourceField("test1", "file.name")},
		},
		{
			name:     "ResourceDoubleBracketedField",
			input:    []byte(`"resource.test1['file.details']['file.name']"`),
			expected: Field{entry.NewResourceField("test1", "file.details", "file.name")},
		},
		{
			name:     "ResourcePostBracketField",
			input:    []byte(`"resource.test1['file.details'].name"`),
			expected: Field{entry.NewResourceField("test1", "file.details", "name")},
		},
		{
			name:     "ResourceSimpleField",
			input:    []byte(`"resource.test1"`),
		...
		},
		{
			...
			expected:    Field{entry.NewAttributeField()},
		},
		{
			name:        "ResourceRoot",
			input:       []byte(`"resource"`),
			expectedErr: "resource cannot be referenced without subfield",
			expected:    Field{entry.NewResourceField()},
		},
		{
			name:                "Bool",
			input:               []byte(`"bool"`),
