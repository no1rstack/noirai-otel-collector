...
func (cmd *asyncUp) Up(ctx context.Context) error {
	...
	err = cmd.run(ctx, schemamigrator.TracesMigrations, schemamigrator.NoirAITracesDB)
	if err != nil {
		return err
	}
	...
	err = cmd.run(ctx, logsMigrations, schemamigrator.NoirAILogsDB)
	if err != nil {
		return err
	}

	err = cmd.run(ctx, schemamigrator.MetricsMigrations, schemamigrator.NoirAIMetricsDB)
	if err != nil {
		return err
	}

	err = cmd.run(ctx, schemamigrator.MetadataMigrations, schemamigrator.NoirAIMetadataDB)
	if err != nil {
		return err
	}

	err = cmd.run(ctx, schemamigrator.AnalyticsMigrations, schemamigrator.NoirAIAnalyticsDB)
	if err != nil {
		return err
	}

	err = cmd.run(ctx, schemamigrator.MeterMigrations, schemamigrator.NoirAIMeterDB)
	if err != nil {
		return err
	}
...
func (cmd *asyncUp) runSquashedMigrations(ctx context.Context) error {
	squashedMigrations := map[string][]schemamigrator.SchemaMigrationRecord{
		schemamigrator.NoirAILogsDB:    schemamigrator.CustomRetentionLogsMigrations,
		schemamigrator.NoirAIMetricsDB: schemamigrator.SquashedMetricsMigrations,
		schemamigrator.NoirAITracesDB:  schemamigrator.SquashedTracesMigrations,
	}
