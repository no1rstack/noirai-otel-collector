...
var (
	...
	NoirAILogsDB          = "noirai_logs"
	NoirAIMetricsDB       = "noirai_metrics"
	NoirAITracesDB        = "noirai_traces"
	NoirAIMetadataDB      = "noirai_metadata"
	NoirAIAnalyticsDB     = "noirai_analytics"
	NoirAIMeterDB         = "noirai_meter"
	Databases             = []string{NoirAITracesDB, NoirAIMetricsDB, NoirAILogsDB, NoirAIMetadataDB, NoirAIAnalyticsDB, NoirAIMeterDB}
...
func (m *MigrationManager) Bootstrap() error {
	...
	for _, migration := range V2MigrationTablesLogs {
		for _, item := range migration.UpItems {
			if err := m.RunOperation(context.Background(), item, migration.MigrationID, NoirAILogsDB, true); err != nil {
	...
	for _, migration := range V2MigrationTablesMetrics {
		for _, item := range migration.UpItems {
			if err := m.RunOperation(context.Background(), item, migration.MigrationID, NoirAIMetricsDB, true); err != nil {
				return errors.Join(ErrFailedToCreateSchemaMigrationsV2, err)
			}
		}
	}
	...
	for _, migration := range V2MigrationTablesTraces {
		for _, item := range migration.UpItems {
			if err := m.RunOperation(context.Background(), item, migration.MigrationID, NoirAITracesDB, true); err != nil {
				return errors.Join(ErrFailedToCreateSchemaMigrationsV2, err)
			}
		}
	}
	...
	for _, migration := range V2MigrationTablesMetadata {
		for _, item := range migration.UpItems {
			if err := m.RunOperation(context.Background(), item, migration.MigrationID, NoirAIMetadataDB, true); err != nil {
				return errors.Join(ErrFailedToCreateSchemaMigrationsV2, err)
			}
		}
	}
	...
	for _, migration := range V2MigrationTablesAnalytics {
		for _, item := range migration.UpItems {
			if err := m.RunOperation(context.Background(), item, migration.MigrationID, NoirAIAnalyticsDB, true); err != nil {
				return errors.Join(ErrFailedToCreateSchemaMigrationsV2, err)
			}
		}
	}
	...
	for _, migration := range V2MigrationTablesMeter {
		for _, item := range migration.UpItems {
			if err := m.RunOperation(context.Background(), item, migration.MigrationID, NoirAIMeterDB, true); err != nil {
				return errors.Join(ErrFailedToCreateSchemaMigrationsV2, err)
			}
		}
	}
...
func (m *MigrationManager) runSquashedMigrationsForMetrics(ctx context.Context) error {
	...
	should, err := m.shouldRunSquashed(ctx, NoirAIMetricsDB)
	if err != nil {
		return err
	}
	if !should {
	...
	for _, migration := range SquashedMetricsMigrations {
		for _, item := range migration.UpItems {
			if err := m.RunOperation(ctx, item, migration.MigrationID, NoirAIMetricsDB, false); err != nil {
				return err
			}
		}
	}
...
func (m *MigrationManager) runSquashedMigrationsForTraces(ctx context.Context) error {
	...
	should, err := m.shouldRunSquashed(ctx, NoirAITracesDB)
	if err != nil {
		return err
	}
	if !should {
	...
	for _, migration := range SquashedTracesMigrations {
		for _, item := range migration.UpItems {
			if err := m.RunOperation(ctx, item, migration.MigrationID, NoirAITracesDB, false); err != nil {
				return err
			}
		}
	}
		...
		if err := m.conn.Select(ctx, &ddlQueue, query); err != nil {
			if exception, ok := err.(*clickhouse.Exception); ok {
				if exception.Code == 999 {
					...
					// code: 999, message: Coordination error: No node, path /clickhouse/noirai-clickhouse/task_queue/ddl/query-000000<some 4 digit number>/finished
...
func (m *MigrationManager) MigrateUpSync(ctx context.Context, upVersions []uint64) error {
	...
	for _, migration := range TracesMigrations {
		if !m.shouldRunMigration(NoirAITracesDB, migration.MigrationID, upVersions) {
			continue
		}
		if err := m.executeSyncOperations(ctx, migration.UpItems, migration.MigrationID, NoirAITracesDB); err != nil {
			return err
		}
	}
	...
	for _, migration := range logsMigrations {
		if !m.shouldRunMigration(NoirAILogsDB, migration.MigrationID, upVersions) {
			continue
		}
		if err := m.executeSyncOperations(ctx, migration.UpItems, migration.MigrationID, NoirAILogsDB); err != nil {
			return err
		}
	}

	for _, migration := range MetricsMigrations {
		if !m.shouldRunMigration(NoirAIMetricsDB, migration.MigrationID, upVersions) {
			continue
		}
		if err := m.executeSyncOperations(ctx, migration.UpItems, migration.MigrationID, NoirAIMetricsDB); err != nil {
			return err
		}
	}

	for _, migration := range MetadataMigrations {
		if !m.shouldRunMigration(NoirAIMetadataDB, migration.MigrationID, upVersions) {
			continue
		}
		if err := m.executeSyncOperations(ctx, migration.UpItems, migration.MigrationID, NoirAIMetadataDB); err != nil {
			return err
		}
	}

	for _, migration := range AnalyticsMigrations {
		if !m.shouldRunMigration(NoirAIAnalyticsDB, migration.MigrationID, upVersions) {
			continue
		}
		if err := m.executeSyncOperations(ctx, migration.UpItems, migration.MigrationID, NoirAIAnalyticsDB); err != nil {
			return err
		}
	}

	for _, migration := range MeterMigrations {
		if !m.shouldRunMigration(NoirAIMeterDB, migration.MigrationID, upVersions) {
			continue
		}
		if err := m.executeSyncOperations(ctx, migration.UpItems, migration.MigrationID, NoirAIMeterDB); err != nil {
			return err
		}
	}
...
func (m *MigrationManager) MigrateDownSync(ctx context.Context, downVersions []uint64) error {
	...
	for _, migration := range TracesMigrations {
		if !m.shouldRunMigration(NoirAITracesDB, migration.MigrationID, downVersions) {
			continue
		}
		if err := m.executeSyncOperations(ctx, migration.DownItems, migration.MigrationID, NoirAITracesDB); err != nil {
			return err
		}
	}
	...
	for _, migration := range logsMigrations {
		if !m.shouldRunMigration(NoirAILogsDB, migration.MigrationID, downVersions) {
			continue
		}
		if err := m.executeSyncOperations(ctx, migration.DownItems, migration.MigrationID, NoirAILogsDB); err != nil {
			return err
		}
	}

	for _, migration := range MetricsMigrations {
		if !m.shouldRunMigration(NoirAIMetricsDB, migration.MigrationID, downVersions) {
			continue
		}
		if err := m.executeSyncOperations(ctx, migration.DownItems, migration.MigrationID, NoirAIMetricsDB); err != nil {
			return err
		}
	}

	for _, migration := range AnalyticsMigrations {
		if !m.shouldRunMigration(NoirAIAnalyticsDB, migration.MigrationID, downVersions) {
			continue
		}
		if err := m.executeSyncOperations(ctx, migration.DownItems, migration.MigrationID, NoirAIAnalyticsDB); err != nil {
			return err
		}
	}

	for _, migration := range MeterMigrations {
		if !m.shouldRunMigration(NoirAIMeterDB, migration.MigrationID, downVersions) {
			continue
		}
		if err := m.executeSyncOperations(ctx, migration.DownItems, migration.MigrationID, NoirAIMeterDB); err != nil {
			return err
		}
	}
...
func (m *MigrationManager) MigrateUpAsync(ctx context.Context, upVersions []uint64) error {
	...
	for _, migration := range logsMigrations {
		...
		for _, item := range migration.UpItems {
			...
			if item.IsIdempotent() {
				if err := m.RunOperation(ctx, item, migration.MigrationID, NoirAILogsDB, false); err != nil {
					return err
				}
			}
		}
